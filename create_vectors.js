...
function preprocessInput(img)
{
    ...
    let processedValues = {
        red: tf.gather(img, indices[0], 2).sub(tf.scalar(mean[0])).div(tf.scalar(std[0])).reshape([imageSize*imageSize]),
        green: tf.gather(img, indices[1], 2).sub(tf.scalar(mean[1])).div(tf.scalar(std[1])).reshape([imageSize*imageSize]),
        blue: tf.gather(img, indices[2], 2).sub(tf.scalar(mean[2])).div(tf.scalar(std[2])).reshape([imageSize*imageSize]),
    };

    let processedTensor = tf.stack([
        processedValues.red, processedValues.green, processedValues.blue
    ], 1, "float32").reshape([imageSize, imageSize, 3]).expandDims();
